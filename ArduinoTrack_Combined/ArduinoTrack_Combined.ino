...
ArduinoTrack_Combined is designed for use with the Arduino 1.5.8+ development environment.  This is a combined AX.25 modulator as well
as the APRS logic board for generating packets.

Step 1:
Be sure you have the Arduino 1.5.8+ development environment installed.  These instructions are specifically for the 1.5+ version of Arduino.
...
Step 6:
Plug the ArduinoTrack_Combined shield into Uno that is programmed as an ISP programmer.
...
Open a command prompt on the PC.
  ...
  * Replace "com24" with whatever Com port your ArduinoISP is enumerating on.
  ...
  C:\Program Files (x86)\Arduino\hardware\tools\avr\bin> avrdude.exe -p m328p  -b 19200 -c avrisp -C ..\etc\avrdude.conf -P com34 -U hfuse:r:highfuse:h -U lfuse:r:lowfuse:h -U efuse:r:exfuse:h
...
struct udtConfig {
  ...
  byte BeaconSimpleDelay;      //TODO: Switch this to unsigned long integer so that it can hold more than 255 second delays.
